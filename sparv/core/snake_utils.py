        ...
        if self._source_files is None:
            ...
            try:
                ...
                raise SparvErrorMessage(
                    f"""Could not find the importer '{sparv_config.get("import.importer")}'. Make sure the 'import.importer' config value refers to an existing importer.""",
                    "sparv",
                )
            ...
            if wrong_ext := [
                f[1][0]
                for f in sf
                if not f[1][0].endswith(file_extension) and not Path(f[0]).is_dir()
            ]:
