...
class MySQL:
    ...
    def create_table(self, table, drop, columns, primary=None, indexes=None, constraints=None, **kwargs):
        ...
        if constraints:
            for constraint in constraints:
                sqlcolumns += [
                    f"CONSTRAINT {constraint[0]} {_atom(constraint[1])} ({_atomseq(constraint[2])})"
                ]
        if drop:
    ...
    def lock(self, *tables):
        t = ", ".join([f"{_atom(table)} WRITE" for table in tables])
    ...
    def delete_rows(self, table, conditions):
        conditions = " AND ".join(
            [f"{_atom(k)} = {_value(v)}" for (k, v) in list(conditions.items())]
        )
    ...
    def rename_table(self, tables):
        renames = [
            f"{_atom(old)} TO {_atom(new)}" for old, new in list(tables.items())
        ]
...
def _value(val):
    ...
    if val is None:
        return "NULL"
    return f"'{_escape(val)}'" if isinstance(val, str) else f"{val}"
